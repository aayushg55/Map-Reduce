...
async fn test_wc() -> Result<()> {
    ...
    let mut client =
        coordinator_client::CoordinatorClient::connect(format!("http://{}", COORDINATOR_ADDR))
            .await?;
    let infile = [
        "data/gutenberg/p.txt".to_string(),
        "data/gutenberg/q.txt".to_string(),
        "data/gutenberg/r.txt".to_string(),
        "data/gutenberg/s.txt".to_string()
      ].to_vec();
    ...
    .await?;
    // let r = poll_job(&mut client, job_id+1).await?;
